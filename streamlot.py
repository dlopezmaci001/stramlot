...
def fast_server_nimerya(database_name,df,tablename,if_exists):
   ...
   .to_sql will create the table in the SQL server, therefore the column names of
    ...
    """       
    conn = pyodbc.connect('DRIVER={/opt/microsoft/msodbcsql17/lib64/libmsodbcsql-17.3.so.1.1}; + SERVER='+ "'"+st.secrets["DB_SERVER"]+ "'" +
                                     ';DATABASE='+'bimbo'+
...
def main():
    ...
    st.title([item for item in pyodbc.drivers()])
    # Side bar 
